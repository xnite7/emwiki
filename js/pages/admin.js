/* Admin panel app. Extracted verbatim from admin.html's inline <script>
   (classic script on purpose: inline onclick handlers rely on its top-level
   declarations being global). */
        ...
        const DEV_MODE = {
            enabled: window.location.href.startsWith('file:///C:/Users/ADMIN/Desktop/EpicCatalogue/emwiki/'),
            adminName: 'dev_admin'
        };
        ...
        const ChangeTracker = {
            ...
            checkModifications(item, oldName = null) {
                ...
                let originalJson = this.originalItems.get(itemName);
                if (!originalJson) {
                    // If original not found, capture current state as original (deep copy)
                    // This handles cases where items weren't captured during initialization
                    console.warn(`No original found for ${itemName}, capturing current state`);
                    const currentCopy = JSON.parse(JSON.stringify(item));
                    originalJson = JSON.stringify(currentCopy);
                    this.originalItems.set(itemName, originalJson);
                    // Since we just captured current state as original, no changes detected yet
                    // But mark as modified anyway since we don't know the true original state
                    // This ensures drag operations are tracked
            ...
            compareItems(original, current) {
                ...
                const fields = ['name', 'img', 'svg', 'price', 'from', 'price/code/rarity',
                    'weekly', 'weeklystar', 'new', 'retired', 'premium', 'tradable', 'removed', 'unstable', 'demand', 'demand_updated_at', 'credits', 'lore', 'alias', 'quantity'];

                // Boolean fields that need normalization
                const booleanFields = ['weekly', 'weeklystar', 'new', 'retired', 'premium', 'tradable', 'removed', 'unstable'];

                // Check each field for changes
                for (const field of fields) {
                    // Use 'in' operator to check if field exists, or get undefined
                    let val1 = original.hasOwnProperty(field) ? original[field] : undefined;
                    let val2 = current.hasOwnProperty(field) ? current[field] : undefined;
                    ...
                    // Normalize boolean fields
                        ...
                        val1 = normalizeBoolean(val1);
                        val2 = normalizeBoolean(val2);
                    }
                    // Normalize demand field (it's a number)
                    else if (field === 'demand') {
                        val1 = parseInt(val1) || 0;
                        val2 = parseInt(val2) || 0;
                    }
                    // Normalize demand_updated_at field (it's a number/timestamp)
                    else if (field === 'demand_updated_at') {
                        // Handle null/undefined - convert to null for comparison
                        val1 = (val1 === null || val1 === undefined) ? null : (typeof val1 === 'number' ? val1 : parseInt(val1) || null);
                        val2 = (val2 === null || val2 === undefined) ? null : (typeof val2 === 'number' ? val2 : parseInt(val2) || null);
                        
                        // Debug logging for demand_updated_at changes
                        if (val1 !== val2) {
                            console.log(`Demand updated at changed for ${original.name || 'unknown'}`);
                        }
                    }

                    // Convert undefined/null to empty string for comparison (only for non-boolean, non-number fields)
                    if (!booleanFields.includes(field) && field !== 'demand' && field !== 'demand_updated_at' && field !== 'price') {
                ...
                // Compare price history (edited via the Price History editor). Without this,
                // a history-only edit isn't seen as a change and checkModifications would
                // drop the item from the save set.
                const phStr = (arr) => JSON.stringify(
                    (Array.isArray(arr) ? arr : []).map(p => ({
                        price: Number(p.price) || 0,
                        timestamp: Number(p.timestamp) || 0
                    }))
                );
                if (phStr(original.priceHistory) !== phStr(current.priceHistory)) {
                    return true;
                }
        ...
        const UndoRedoManager = {
            ...
            undoAdd(action) {
                ...
                ErrorHandler.showToast(`⮌ Removed: ${action.itemName}`, 'info');
            },
            ...
            undoEdit(action) {
                ...
                ErrorHandler.showToast(`⮌ Restored: ${action.itemName}`, 'info');
            },
            ...
            redoAdd(action) {
                ...
                ErrorHandler.showToast(`⮎ Re-added: ${action.itemName}`, 'info');
            },
            ...
            redoEdit(action) {
                ...
                ErrorHandler.showToast(`⮎ Re-applied: ${action.itemName}`, 'info');
            },
            ...
            redoDelete(action) {
                ...
                ErrorHandler.showToast(`⮎ Re-deleted: ${action.itemName}`, 'info');
            },
        ...
        const ItemAdder = {
            ...
            createModal() {
                ...
                this.modal.innerHTML = `<div class=item-adder-container><div class=item-adder-header><h2>Add New Item</h2><button class=close-btn id=item-adder-close>×</button></div><div class=item-adder-steps><div class="active step"data-step=1><span class=step-number>1</span> <span class=step-label>Category</span></div><div class=step data-step=2><span class=step-number>2</span> 
                            ...
                            <div class="form-group" id="image-upload-group">
                                ...
                                <div class=image-upload-section>
                                    <div class=image-preview id=image-preview>
                                        ...
                                        <button class=remove-image-btn id=remove-image style=display:none>×</button>
                                    </div>
            ...
            async handleFileUpload(file) {
                ...
                try {
                    ...
                    const response = await fetch('/api/images/upload', {
                        ...
                        body: formData,
                        headers: {
                            'Authorization': `Bearer ${Auth.authToken || localStorage.getItem('auth_token')}`
                        }
                    });
                    ...
                    if (!data.success && !data.url) {
                        ErrorHandler.showToast(data.error || 'Upload failed', 'error');
                        return;
                    }

                    // Cloudflare Images returns { success: true, url: "...", id: "..." }
    ...
    `;
            ...
            addItem() {
                ...
                ErrorHandler.showToast(`✓ Added "${newItem.name}" to ${category}`, 'success');
        ...
        const API = {
            ...
            async loadData() {
                try {
                    ...
                    const categoryPromises = categories.map(async (category) => {
                        ...
                        state.jsData[category] = categoryItems;
                        state.allItems.push(...categoryItems);
                    });
            ...
            async loadDemandRatings() {
                // Demand is now part of the items table, so it's already loaded with items
                // This function is kept for backward compatibility but doesn't need to fetch separately
                // Demand and demand_updated_at are already included in the items from the API
                state.allItems = Object.values(state.jsData).flat();
                console.log('Demand ratings loaded with items');
            ...
            async saveData() {
                ...
                // Reconstruct old state from originalItems for accurate diff calculation
                // This ensures we compare against the true original state, not the current modified state
                const oldItemsByCategory = {};
                
                // Reconstruct old state from originalItems (true original state from when data was loaded)
                ChangeTracker.originalItems.forEach((originalJson, itemName) => {
                    const originalItem = JSON.parse(originalJson);
                    const category = originalItem.category;
                    if (!oldItemsByCategory[category]) {
                        oldItemsByCategory[category] = [];
                    }
                    // Only add if not already present (in case of duplicates)
                    const exists = oldItemsByCategory[category].some(item => item.name === originalItem.name);
                    if (!exists) {
                        oldItemsByCategory[category].push(originalItem);
                    }
                });
                
                // Add deleted items to old state for accurate diff
                ...
                // Save items to D1 database individually
                // Only save items that have actually changed (new, modified, or deleted)
                const allItemsToSave = [];
                const itemsToDelete = [];
                
                // Collect new and modified items
                const changedItemNames = new Set([
                    ...ChangeTracker.newItems,
                    ...ChangeTracker.modifiedItems
                ]);
                
                Object.entries(cleanedData).forEach(([category, items]) => {
                    if (Array.isArray(items)) {
                        items.forEach(item => {
                            // Only save if this item has changed
                            if (changedItemNames.has(item.name)) {
                                allItemsToSave.push({
                                    ...item,
                                    category: category,
                                    ...
                                    // Preserve updated_at from when item was loaded for optimistic locking
                                    // Only include updated_at for modified items (not new items)
                                    updated_at: ChangeTracker.newItems.has(item.name) ? null : (item.updated_at || null)
                                });
                            }
                        });
                    }
                ...
                console.log(`Saving ${allItemsToSave.length} changed items and ${itemsToDelete.length} deletions to D1 database...`);

                // Save items in batches to avoid overwhelming the API
                const batchSize = 50;
                let successCount = 0;
                let errorCount = 0;
                const errors = [];
                
                // If no items to save or delete, return early
                if (allItemsToSave.length === 0 && itemsToDelete.length === 0) {
                    console.warn('No items to save or delete');
                    return {
                        ok: true,
                        status: 200,
                ...
                const authToken = Auth.authToken || localStorage.getItem('auth_token');
                const authHeaders = authToken
                    ? { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authToken}` }
                    : { 'Content-Type': 'application/json' };

                const buildItemPayload = (item, { includeUpdatedAt }) => {
                    const payload = {
                        name: item.name,
                        ...
                        from: item.from || null,
                        price_code_rarity: item.price_code_rarity,
                    ...
                    // When the Price History editor was used, tell the API to replace the
                    // stored history outright instead of merging (so deletions/edits stick).
                    if (item._replacePriceHistory) {
                        payload.replace_price_history = true;
                    }
                    return payload;
                };

                for (let i = 0; i < allItemsToSave.length; i += batchSize) {
                    const batch = allItemsToSave.slice(i, i + batchSize);

                    const batchPromises = batch.map(async (item) => {
                        try {
                            const isNew = ChangeTracker.newItems.has(item.name) || !item.id;

                            if (isNew) {
                                const createRes = await fetch('/api/items', {
                                    method: 'POST',
                                    headers: authHeaders,
                                    body: JSON.stringify(buildItemPayload(item, { includeUpdatedAt: false }))
                                });

                                if (!createRes.ok) {
                                    const errorText = await createRes.text();
                                    throw new Error(`Failed to create ${item.name}: ${errorText}`);
                                }
                            } else {
                                // Detect manual demand_updated_at edit (Keep Demand button) so the server
                                // refreshes the timestamp even if demand value didn't change.
                                let forceUpdateDemandTimestamp = false;
                                const originalJson = ChangeTracker.originalItems.get(item.name);
                                if (originalJson) {
                                    try {
                                        const originalItem = JSON.parse(originalJson);
                                        const demandChanged = (item.demand || 0) !== (originalItem.demand || 0);
                                        const demandUpdatedAtChanged = item.demand_updated_at !== originalItem.demand_updated_at;
                                        if (demandUpdatedAtChanged && !demandChanged) {
                                            forceUpdateDemandTimestamp = true;
                                        }
                                    } catch (e) {
                                        console.error('Error parsing original item:', e);
                                    }
                                }

                                const payload = buildItemPayload(item, { includeUpdatedAt: true });
                                if (forceUpdateDemandTimestamp) {
                                    payload.force_update_demand_timestamp = true;
                                }

                                const updateRes = await fetch(`/api/items/${item.id}`, {
                                    method: 'PUT',
                                    headers: authHeaders,
                                ...
                                if (updateRes.status === 409) {
                                    ...
                                    errors.push({
                                        item: item.name,
                                        error: `Conflict: Modified by another admin. Server version has different data.`,
                                        conflict: true,
                                        currentItem: conflictData.currentItem
                                    });
                                    errorCount++;
                                    return;
                                }
                                ...
                                if (updateRes.status === 404) {
                                    throw new Error(`Item id ${item.id} not found on server (${item.name})`);
                                }

                                if (!updateRes.ok) {
                                    const errorText = await updateRes.text();
                                    throw new Error(`Failed to update ${item.name}: ${errorText}`);
                                }
                            }

                            // History edit persisted — clear the replace flag so future
                            // plain price changes resume normal auto-append behavior.
                            if (item._replacePriceHistory) {
                                delete item._replacePriceHistory;
                            }
                ...
                for (const deletedItem of itemsToDelete) {
                    try {
                        if (!deletedItem.id) {
                            // No id captured at load time — nothing to delete server-side.
                            // (Happens if a brand-new local item was added and removed in the same session.)
                            successCount++;
                            continue;
                        }

                        const deleteRes = await fetch(`/api/items/${deletedItem.id}`, {
                            method: 'DELETE',
                            headers: authHeaders
                        });
                        ...
                        if (!deleteRes.ok) {
                            ...
                            throw new Error(`Failed to delete ${deletedItem.name}: ${errorText}`);
                        }
                        successCount++;
                    } catch (error) {
                        errorCount++;
                        errors.push({ item: deletedItem.name, error: error.message });
                        console.error(`Error deleting item ${deletedItem.name}:`, error);
                    }
                }

                // Note: Demand ratings are now saved as part of item updates above
                // No separate bulk save needed since demand is part of the items table

                // Check if there were any conflicts
                const conflicts = errors.filter(e => e.conflict);
                const hasConflicts = conflicts.length > 0;
                
                console.log(`Save completed: ${successCount} saved, ${errorCount} errors`);
                ...
                if (errorCount > 0) {
                    ...
                    return {
                        ok: false,
                        status: hasConflicts ? 409 : 500,
                        text: async () => JSON.stringify({ 
                            error: hasConflicts
                                ? `Conflict: ${conflicts.length} item(s) were modified by another admin. Please reload and try again.`
                                : `Failed to save ${errorCount} item(s)`,
                            errors: errors,
                            conflicts: hasConflicts ? conflicts : undefined,
                            stats: {
                                total: allItemsToSave.length + itemsToDelete.length,
                                saved: successCount,
                                errors: errorCount
                            }
                        })
                ...
                // Only return success if we actually saved something or had items to process
                const totalItemsProcessed = allItemsToSave.length + itemsToDelete.length;
                ...
                console.log(`Successfully saved ${successCount} items`);

                // Calculate diff and save history
                if (successCount > 0) {
                    try {
                        const newItemsByCategory = JSON.parse(JSON.stringify(state.jsData));
                        const diff = this.calculateReadableDiff(oldItemsByCategory, newItemsByCategory);
                        
                        // Get username from state - use adminName which is set during login
                        const username = state.adminName || state.currentUser?.username || state.username || 'unknown';
                        ...
                        console.log('Saving history:', {
                            ...
                            oldItemsCount: Object.values(oldItemsByCategory).flat().length,
                            newItemsCount: Object.values(newItemsByCategory).flat().length
                        });
                        
                        // Save history entry (even if diff shows no changes, we still record the save)
                        await this.saveHistory(username, diff || '(No changes detected)', null);
                ...
                return {
                    ...
                    text: async () => JSON.stringify({ 
                        success: true,
                        message: `Successfully saved ${successCount} item(s)`,
                        stats: {
            ...
            // Note: saveDemandRatings() removed - demand is now saved as part of item updates
            // Demand changes are tracked in ChangeTracker and saved when items are updated
            // This ensures demand is always in sync with item data
            ...
            updatePriceHistory() {
                ...
                state.allItems.forEach(item => {
                    ...
                    // The Price History editor already set an exact, curated history for
                    // this item — don't auto-append on top of it.
                    if (item._replacePriceHistory) {
                        return;
                    }
                    ...
                    if (isModified || isNew) {
                        ...
                        if (originalJson && !isNew) {
                            const original = JSON.parse(originalJson);
                            oldPrice = original.price;  // ← Use raw price, not normalized
                        }

                        const currentPrice = item.price;  // ← Use raw price
            ...
            async saveHistory(username, diff, version = null) {
                try {
                    const response = await fetch(CONFIG.API_URLS.HISTORY, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ username, diff, version })
                    });
                    if (!response.ok) {
            ...
            // Calculate readable diff from old and new item states
            calculateReadableDiff(oldItemsByCategory, newItemsByCategory) {
                const output = [];
                // Get all categories from both old and new states
                const allCategories = new Set([
                    ...Object.keys(oldItemsByCategory),
                    ...Object.keys(newItemsByCategory)
                ]);

                for (const category of allCategories) {
                    const newItems = newItemsByCategory[category] || [];
                    const oldItems = oldItemsByCategory[category] || [];

                    // Create maps for easier lookup
                    const oldItemsMap = new Map();
                    oldItems.forEach(item => oldItemsMap.set(item.name, item));
                    ...
                    for (const newItem of newItems) {
                        const oldItem = oldItemsMap.get(newItem.name);
                        if (!oldItem) {
                        ...
                        const fields = ['name', 'img', 'svg', 'price', 'from', 'price/code/rarity',
                            'weekly', 'weeklystar', 'new', 'retired', 'premium', 'tradable', 'removed', 'demand', 'credits', 'lore', 'alias', 'quantity'];
                        ...
                        for (const field of fields) {
                            ...
                            if (field === 'price') {
                                normalizedOld = (!oldVal || oldVal === '' || oldVal === 'N/A' || oldVal === 'undefined') ? '0' : String(oldVal);
                                normalizedNew = (!newVal || newVal === '' || newVal === 'N/A' || newVal === 'undefined') ? '0' : String(newVal);
                            } else if (['weekly', 'weeklystar', 'new', 'retired', 'premium', 'tradable', 'removed'].includes(field)) {
                                // Boolean fields
                                normalizedOld = oldVal === true || oldVal === 1 || oldVal === 'true' ? 'true' : 'false';
                                normalizedNew = newVal === true || newVal === 1 || newVal === 'true' ? 'true' : 'false';
                            } else {
                                normalizedOld = (oldVal === undefined || oldVal === null) ? '' : String(oldVal);
                                normalizedNew = (newVal === undefined || newVal === null) ? '' : String(newVal);
                            }

                            if (normalizedOld !== normalizedNew) {
                                output.push(`[${newItem.name}] ${field}: ${normalizedOld} → ${normalizedNew}`);
                            }
                        }
                    }
                    ...
                    for (const oldItem of oldItems) {
                        if (!newItemsMap.has(oldItem.name)) {
        ...
        const UI = {
            ...
            renderFilterControls() {
                ...
                const descSearchLabel = document.createElement('label');
                descSearchLabel.style.cssText = 'display: flex; align-items: center; gap: 6px; font-size: 13px; color: #999; cursor: pointer; white-space: nowrap;';
                const descSearchCheckbox = document.createElement('input');
                descSearchCheckbox.type = 'checkbox';
                descSearchCheckbox.id = 'search-descriptions';
                descSearchCheckbox.checked = state.filters.searchDescriptions;
                descSearchCheckbox.style.cursor = 'pointer';
                descSearchCheckbox.addEventListener('change', (e) => {
                    state.filters.searchDescriptions = e.target.checked;
                    UI.renderItems();
                });
                descSearchLabel.appendChild(descSearchCheckbox);
                descSearchLabel.appendChild(document.createTextNode('Search descriptions'));
                ...
                container.appendChild(descSearchLabel);
            ...
            renderItems(keepPage = false) {
                ...
                if (state.filters.search) {
                    const searchKeys = state.filters.searchDescriptions ? ['from'] : ['name'];
                    ...
                    console.log(fuse.search(state.filters.search));
                    console.log(fuse.search(state.filters.search).map(r => r.item));
                }
                ...
                new Sortable(DOM.get('all-items'), {
                    ...
                    onAdd(evt) {
                        ...
                        for (const arr of Object.values(state.jsData)) {
                            ...
                            if (it) {
                                // Capture original state BEFORE modifying if not already captured
                                if (!ChangeTracker.originalItems.has(it.name) && !ChangeTracker.isNewItem(it.name)) {
                                    // Deep copy the current state as the original baseline
                                    const originalCopy = JSON.parse(JSON.stringify(it));
                                    ChangeTracker.originalItems.set(it.name, JSON.stringify(originalCopy));
                                }
                                // Modify the item
                                const oldValue = it[key];
                                it[key] = false;
                                // Check modifications - this will compare against the stored original
                                ChangeTracker.checkModifications(it);
                            }
        ...
        // ============================
        // Lets an admin add / edit / delete / clear the individual points behind an
        // item's price graph. Edits are staged on state.currentEditingItem and persisted
        // through the normal global Save flow (with replace_price_history so deletions
        // actually stick — the API merges history otherwise).
        const PriceHistoryEditor = {
            item: null,        // the item being edited (reference into state.allItems)
            working: [],       // working copy of price history points
            chart: null,       // Chart.js instance

            open(item) {
                if (!item) return;
                this.item = item;
                // Deep clone existing history so Cancel discards cleanly.
                this.working = (item.priceHistory || []).map(p => ({
                    price: Number(p.price) || 0,
                    timestamp: Number(p.timestamp) || Date.now(),
                    admin: p.admin || null
                }));

                const nameEl = DOM.get('ph-editor-item-name');
                if (nameEl) nameEl.textContent = item.name || '';

                this.renderList();
                this.renderChart();

                const modal = DOM.get('price-history-editor');
            ...
            bindControls() {
                ...
                this._bound = true;
                const closeBtn = DOM.get('ph-editor-close');
                const cancelBtn = DOM.get('ph-cancel-btn');
                const applyBtn = DOM.get('ph-apply-btn');
                const addBtn = DOM.get('ph-add-point-btn');
                const clearBtn = DOM.get('ph-clear-btn');
                const backdrop = DOM.query('.ph-editor-backdrop');
                ...
                if (addBtn) addBtn.onclick = () => this.addPoint();
                if (clearBtn) clearBtn.onclick = () => this.clearAll();
            },

            // Convert a ms timestamp to a yyyy-mm-dd string for <input type="date">.
            toDateInput(ts) {
            ...
            renderList() {
                const list = DOM.get('ph-list');
                if (!list) return;
                list.innerHTML = '';

                this.working
                    .map((p, i) => ({ p, i }))
                    .sort((a, b) => (a.p.timestamp || 0) - (b.p.timestamp || 0))
                    .forEach(({ p, i }) => {
                        const li = document.createElement('li');

                        const dateInput = document.createElement('input');
                        dateInput.type = 'date';
                        dateInput.value = this.toDateInput(p.timestamp);
                        dateInput.oninput = () => {
                            const parsed = Date.parse(dateInput.value);
                            if (!isNaN(parsed)) {
                                this.working[i].timestamp = parsed;
                                this.renderChart();
                            }
                        };
                        ...
                        priceInput.oninput = () => {
                            this.working[i].price = Number(priceInput.value) || 0;
                            this.renderChart();
                        };

                        const del = document.createElement('button');
                        del.type = 'button';
                        del.className = 'ph-row-delete';
            ...
            renderChart() {
                const canvas = DOM.get('ph-editor-chart');
                const empty = DOM.get('ph-editor-empty');
                if (!canvas) return;

                const points = [...this.working]
                    .filter(p => p.price !== 0 && p.price !== '0')
                    .sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));

                if (this.chart) { this.chart.destroy(); this.chart = null; }

                if (typeof Chart === 'undefined' || points.length < 2) {
                    canvas.style.display = 'none';
                    if (empty) {
                        empty.style.display = 'flex';
                        empty.textContent = (typeof Chart === 'undefined')
                            ? 'Chart library failed to load.'
                            : 'Add at least 2 non-zero points to preview the graph.';
                    }
                    return;
                }
                ...
                this.chart = new Chart(canvas.getContext('2d'), {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                callbacks: {
                                    label: (ctx) => `Price: ${Number(ctx.parsed.y).toLocaleString()}`
                                }
                            }
                        },
                        scales: {
            ...
            apply() {
                ...
                // Normalize: drop NaN prices, sort by timestamp.
                const cleaned = this.working
                    .map(p => ({
                        price: Math.round(Number(p.price)) || 0,
                        timestamp: Number(p.timestamp) || Date.now(),
                        admin: p.admin || state.adminName || null
                    }))
                    .filter(p => Number.isFinite(p.price) && Number.isFinite(p.timestamp))
                    .sort((a, b) => a.timestamp - b.timestamp);

                this.item.priceHistory = cleaned;
                // Signal the save flow to REPLACE (not merge) the stored history so that
                // deletions and edits actually take effect server-side.
                this.item._replacePriceHistory = true;

                ChangeTracker.modifiedItems.add(this.item.name);
                if (ChangeTracker.updateHighlights) ChangeTracker.updateHighlights();
                if (typeof ErrorHandler !== 'undefined' && ErrorHandler.showToast) {
                    ErrorHandler.showToast('Price history updated — click Save to apply', 'success');
                }
        ...
        const QuickEditor = {
            ...
            open(item) {
                ...
                // Setup credits and lore
                this.setupCreditsAndLore(item);
    ...
    `;
            ...
            deleteItem() {
                ...
                const deleteBtn = DOM.get('delete-item-btn');
                if (!deleteBtn) return;

                if (!deleteBtn.dataset.confirming) {
                    deleteBtn.dataset.confirming = 'true';
                    deleteBtn.innerHTML = `<span style="font-weight:600;">Confirm Delete "${item.name}"?</span>`;
                    deleteBtn.style.background = 'linear-gradient(180deg, #ff2020, #890000)';

                    setTimeout(() => {
                        if (deleteBtn.dataset.confirming) {
                            delete deleteBtn.dataset.confirming;
                            deleteBtn.innerHTML = `<span class="icon" style="mask-image: url('${CONFIG.ICONS.delete}'); width: 16px; height: 16px; background: white;"></span> Delete Item`;
                ...
                const category = Object.entries(state.jsData).find(([cat, items]) =>
                    Array.isArray(items) && items.some(i => i === item)
                );

                if (!category) {
                    ErrorHandler.showToast('Could not find item category', 'error');
                    return;
                }
                ...
                const previousState = JSON.parse(JSON.stringify(item));
                const wasNew = ChangeTracker.isNewItem(item.name);

                categoryItems.splice(itemIndex, 1);
                state.allItems = Object.values(state.jsData).flat();

                if (wasNew) {
                    ChangeTracker.newItems.delete(item.name);
                } else {
                    ChangeTracker.markAsDeleted(item.name, JSON.stringify(previousState));
                }
                ...
                UI.renderShops();
                ChangeTracker.updateHighlights();
                ErrorHandler.showToast(`Deleted: ${previousState.name}`, 'success');
            },
            ...
            setupButtons() {
                ...
                const creditsBtn = DOM.get('add-credits-btn');
                const loreBtn = DOM.get('add-lore-btn');
                ...
                if (creditsBtn) creditsBtn.onclick = () => this.startEditingCredits();
                if (loreBtn) loreBtn.onclick = () => this.startEditingLore();
                const aliasBtn = DOM.get('add-alias-btn');
                const quantityBtn = DOM.get('add-quantity-btn');
                if (aliasBtn) aliasBtn.onclick = () => this.startEditingAlias();
                if (quantityBtn) quantityBtn.onclick = () => this.startEditingQuantity();

                const priceHistoryBtn = DOM.get('open-price-history-btn');
                if (priceHistoryBtn) priceHistoryBtn.onclick = () => PriceHistoryEditor.open(state.currentEditingItem);
            ...
            setupCreditsAndLore(item) {
                const creditsContainer = DOM.get('credits-container');
                const loreContainer = DOM.get('lore-container');
                const aliasContainer = DOM.get('alias-container');
                const quantityContainer = DOM.get('quantity-container');
                const creditsDisplay = DOM.get('credits-display');
                const loreDisplay = DOM.get('lore-display');
                const aliasDisplay = DOM.get('alias-display');
                const quantityDisplay = DOM.get('quantity-display');
                const creditsInput = DOM.get('quick-editor-credits');
                const loreInput = DOM.get('quick-editor-lore');
                const aliasInput = DOM.get('quick-editor-alias');
                const quantityInput = DOM.get('quick-editor-quantity');
                const creditsEditBtn = DOM.get('credits-edit-btn');
                const loreEditBtn = DOM.get('lore-edit-btn');
                const aliasEditBtn = DOM.get('alias-edit-btn');
                const quantityEditBtn = DOM.get('quantity-edit-btn');
                const creditsDeleteBtn = DOM.get('credits-delete-btn');
                const loreDeleteBtn = DOM.get('lore-delete-btn');
                const aliasDeleteBtn = DOM.get('alias-delete-btn');
                const quantityDeleteBtn = DOM.get('quantity-delete-btn');

                // Show/hide containers based on whether values exist
                if (item.credits && item.credits.trim()) {
                    creditsContainer.style.display = 'block';
                    const isRobloxId = /^\d+$/.test(item.credits.trim());
                    
                    // Add green glow if it's a Roblox user ID
                    if (isRobloxId) {
                        creditsDisplay.style.textShadow = '0 0 8px rgba(36, 255, 90, 0.8), 0 0 12px rgba(36, 255, 90, 0.6)';
                        creditsDisplay.style.color = '#24ff5a';
                    } else {
                    ...
                    Utils.getRobloxUsername(item.credits).then(username => {
                        creditsDisplay.textContent = username;
                    });
                } else {
                ...
                if (item.lore && item.lore.trim()) {
                    ...
                    loreDisplay.textContent = item.lore.length > 50 ? item.lore.substring(0, 50) + '...' : item.lore;
                } else {
                ...
                if (item.alias && item.alias.trim()) {
                    ...
                    aliasDisplay.textContent = item.alias;
                } else {
                ...
                if (item.quantity && item.quantity.trim()) {
                    ...
                    let quantityValue = item.quantity.trim();
                    
                    // Extract badge ID from Roblox badge URL if stored as URL
                    // URL format: https://www.roblox.com/badges/{badgeId}/{badgeName}
                    const badgeUrlMatch = quantityValue.match(/roblox\.com\/badges\/(\d{10})/i);
                    if (badgeUrlMatch) {
                        quantityValue = badgeUrlMatch[1];
                        // Update the item to store just the ID
                        item.quantity = quantityValue;
                    }
                    
                    const isBadgeId = /^\d{10}$/.test(quantityValue);
                    const isGroup = quantityValue.toLowerCase() === 'group';
                    
                    // Add green glow if it's a valid badge ID (10 digits)
                    if (isBadgeId) {
                        quantityDisplay.style.textShadow = '0 0 8px rgba(36, 255, 90, 0.8), 0 0 12px rgba(36, 255, 90, 0.6)';
                        quantityDisplay.style.color = '#24ff5a';
                        quantityDisplay.textContent = quantityValue + ' (validating...)';
                        
                        // Fetch badge data to validate and show badge name (via proxy to avoid CORS)
                        fetch(`/api/roblox-proxy?mode=badge&badgeId=${quantityValue}`)
                            .then(res => res.ok ? res.json() : null)
                            .then(badgeData => {
                                if (badgeData && badgeData.name) {
                                    quantityDisplay.textContent = `${quantityValue} (${badgeData.name}: ${badgeData.statistics?.awardedCount?.toLocaleString() || 0})`;
                                    quantityDisplay.title = `Badge: ${badgeData.name}\nAwarded: ${badgeData.statistics?.awardedCount?.toLocaleString() || 0}`;
                                } else {
                                    // Invalid badge ID - remove green indicator
                                    quantityDisplay.style.textShadow = '';
                                    quantityDisplay.style.color = 'white';
                                    quantityDisplay.textContent = quantityValue;
                                    quantityDisplay.title = 'Invalid badge ID';
                                }
                            })
                            .catch(() => {
                                // Fetch failed - remove green indicator
                                quantityDisplay.style.textShadow = '';
                                quantityDisplay.style.color = 'white';
                                quantityDisplay.textContent = quantityValue;
                                quantityDisplay.title = 'Could not validate badge ID';
                            });
                    } else if (isGroup) {
                        // Add blue glow for group quantity
                        quantityDisplay.style.textShadow = '0 0 8px rgba(100, 149, 237, 0.8), 0 0 12px rgba(100, 149, 237, 0.6)';
                        quantityDisplay.style.color = '#6495ed';
                        quantityDisplay.textContent = 'group (loading...)';
                        
                        // Fetch group data to show member count (via proxy to avoid CORS)
                        fetch(`/api/roblox-proxy?mode=group&groupId=2649054`)
                            .then(res => res.ok ? res.json() : null)
                            .then(groupData => {
                                if (groupData && groupData.memberCount !== undefined) {
                                    quantityDisplay.textContent = `group (${groupData.name || 'Group'}: ${groupData.memberCount.toLocaleString()} members)`;
                                    quantityDisplay.title = `Group: ${groupData.name || 'Group'}\nMembers: ${groupData.memberCount.toLocaleString()}`;
                                } else {
                                    // Group fetch failed - remove blue indicator
                                    quantityDisplay.style.textShadow = '';
                                    quantityDisplay.style.color = 'white';
                                    quantityDisplay.textContent = quantityValue;
                                    quantityDisplay.title = 'Could not fetch group data';
                                }
                            })
                            .catch(() => {
                                // Fetch failed - remove blue indicator
                                quantityDisplay.style.textShadow = '';
                                quantityDisplay.style.color = 'white';
                                quantityDisplay.textContent = quantityValue;
                                quantityDisplay.title = 'Could not fetch group data';
                ...
                // Credits edit button
                creditsEditBtn.onclick = () => {
                    creditsDisplay.parentElement.style.display = 'none';
                    creditsInput.style.display = 'block';
                    creditsInput.value = item.credits || '';
                    creditsInput.focus();
                };

                // Credits delete button
                creditsDeleteBtn.onclick = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    state.currentEditingItem.credits = '';
                    creditsContainer.style.display = 'none';
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange) {
                        UndoRedoManager.recordChange({
                ...
                // Credits input handlers
                const saveCredits = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    const newValue = creditsInput.value.trim();
                    state.currentEditingItem.credits = newValue;
                    
                    if (newValue) {
                        creditsContainer.style.display = 'block';
                        const isRobloxId = /^\d+$/.test(newValue);
                        ...
                        // Fetch username if it's a Roblox ID, otherwise use as-is
                        Utils.getRobloxUsername(newValue).then(username => {
                            creditsDisplay.textContent = username;
                        });
                    } else {
                        creditsContainer.style.display = 'none';
                    ...
                    creditsDisplay.parentElement.style.display = 'flex';
                    creditsInput.style.display = 'none';
                    
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange &&
                        previousState.credits !== newValue) {
                        UndoRedoManager.recordChange({
                            type: 'edit',
                            itemName: state.currentEditingItem.name,
                            previousState: previousState,
                ...
                creditsInput.onblur = saveCredits;
                creditsInput.onkeydown = (e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        saveCredits();
                    } else if (e.key === 'Escape') {
                ...
                // Lore edit button
                loreEditBtn.onclick = () => {
                    loreDisplay.parentElement.style.display = 'none';
                    loreInput.style.display = 'block';
                    loreInput.value = item.lore || '';
                    loreInput.focus();
                };

                // Lore delete button
                loreDeleteBtn.onclick = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    state.currentEditingItem.lore = '';
                    loreContainer.style.display = 'none';
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange) {
                        UndoRedoManager.recordChange({
                ...
                const saveLore = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    const newValue = loreInput.value.trim();
                    state.currentEditingItem.lore = newValue;
                    
                    if (newValue) {
                        loreContainer.style.display = 'block';
                        loreDisplay.textContent = newValue.length > 50 ? newValue.substring(0, 50) + '...' : newValue;
                    } else {
                        loreContainer.style.display = 'none';
                    }
                    
                    loreDisplay.parentElement.style.display = 'flex';
                    loreInput.style.display = 'none';
                    
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange &&
                        previousState.lore !== newValue) {
                        UndoRedoManager.recordChange({
                            type: 'edit',
                            itemName: state.currentEditingItem.name,
                            previousState: previousState,
                ...
                loreInput.onblur = saveLore;
                loreInput.onkeydown = (e) => {
                    if (e.key === 'Escape') {
                        loreDisplay.parentElement.style.display = 'flex';
                        loreInput.style.display = 'none';
                    }
                ...
                // Alias edit button
                aliasEditBtn.onclick = () => {
                    aliasDisplay.parentElement.style.display = 'none';
                ...
                // Alias delete button
                aliasDeleteBtn.onclick = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    state.currentEditingItem.alias = '';
                    aliasContainer.style.display = 'none';
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange) {
                        UndoRedoManager.recordChange({
                ...
                const saveAlias = () => {
                    ...
                    state.currentEditingItem.alias = newValue;
                    
                    if (newValue) {
                        aliasContainer.style.display = 'block';
                        aliasDisplay.textContent = newValue;
                    } else {
                        aliasContainer.style.display = 'none';
                    }
                    
                    aliasDisplay.parentElement.style.display = 'flex';
                    aliasInput.style.display = 'none';
                    
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange &&
                        previousState.alias !== newValue) {
                        UndoRedoManager.recordChange({
                            type: 'edit',
                            itemName: state.currentEditingItem.name,
                            previousState: previousState,
                ...
                quantityDeleteBtn.onclick = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    state.currentEditingItem.quantity = '';
                    quantityContainer.style.display = 'none';
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange) {
                        UndoRedoManager.recordChange({
                ...
                const saveQuantity = () => {
                    const previousState = JSON.parse(JSON.stringify(state.currentEditingItem));
                    let newValue = quantityInput.value.trim();
                    
                    // Extract badge ID from Roblox badge URL if pasted
                    // URL format: https://www.roblox.com/badges/{badgeId}/{badgeName}
                    const badgeUrlMatch = newValue.match(/roblox\.com\/badges\/(\d{9,})/i);
                    if (badgeUrlMatch) {
                        newValue = badgeUrlMatch[1];
                        quantityInput.value = newValue; // Update input to show extracted ID
                    }
                    
                    state.currentEditingItem.quantity = newValue;
                    
                    if (newValue) {
                        quantityContainer.style.display = 'block';
                        const isBadgeId = /^\d{9,}$/.test(newValue);
                        
                        // Add green glow if it's a valid badge ID (10 digits)
                        if (isBadgeId) {
                            quantityDisplay.style.textShadow = '0 0 8px rgba(36, 255, 90, 0.8), 0 0 12px rgba(36, 255, 90, 0.6)';
                            quantityDisplay.style.color = '#24ff5a';
                            quantityDisplay.textContent = newValue + ' (validating...)';
                            
                            // Fetch badge data to validate and show badge name (via proxy to avoid CORS)
                            fetch(`/api/roblox-proxy?mode=badge&badgeId=${newValue}`)
                                .then(res => res.ok ? res.json() : null)
                                .then(badgeData => {
                                    if (badgeData && badgeData.name) {
                                        quantityDisplay.textContent = `${newValue} (${badgeData.name}: ${badgeData.statistics?.awardedCount?.toLocaleString() || 0})`;
                                        quantityDisplay.title = `Badge: ${badgeData.name}\nAwarded: ${badgeData.statistics?.awardedCount?.toLocaleString() || 0}`;
                                    } else {
                                        // Invalid badge ID - remove green indicator
                                        quantityDisplay.style.textShadow = '';
                    ...
                    quantityDisplay.parentElement.style.display = 'flex';
                    quantityInput.style.display = 'none';
                    
                    ChangeTracker.checkModifications(state.currentEditingItem);
                    if (!UndoRedoManager.isApplyingChange &&
                        previousState.quantity !== newValue) {
                        UndoRedoManager.recordChange({
                            type: 'edit',
                            itemName: state.currentEditingItem.name,
                            previousState: previousState,
                ...
                quantityInput.onblur = saveQuantity;
            ...
            startEditingCredits() {
                ...
                creditsDisplay.parentElement.style.display = 'none';
                creditsInput.style.display = 'block';
                creditsInput.value = state.currentEditingItem.credits || '';
                creditsInput.focus();
            },
            ...
            startEditingLore() {
                ...
                loreDisplay.parentElement.style.display = 'none';
                loreInput.style.display = 'block';
                loreInput.value = state.currentEditingItem.lore || '';
                loreInput.focus();
            },
            ...
            startEditingAlias() {
                ...
                aliasDisplay.parentElement.style.display = 'none';
                aliasInput.style.display = 'block';
                aliasInput.value = state.currentEditingItem.alias || '';
                aliasInput.focus();
            },
            ...
            startEditingQuantity() {
                ...
                quantityDisplay.parentElement.style.display = 'none';
                quantityInput.style.display = 'block';
                quantityInput.value = state.currentEditingItem.quantity || '';
                quantityInput.focus();
            },
            ...
            renderDescriptions(item) {
                ...
                descriptions.forEach(desc => {
                    const isGamenight = /Gamenight \([A-Z][a-z]{2}\./.test(desc);
                ...
                if (!isTouch) {
                    Sortable.create(container, {
                        ...
                        sort: true,
                        filter: '.editing, input, button',  // ← ADD 'input, button'
                        preventOnFilter: false,  // ← ADD THIS
            ...
            createDescItem(value = '', isGamenight = false, focus = false) {
                ...
                const edit = () => {
                    ...
                    li.setAttribute('data-draggable', 'false');  // ← ADD THIS
                ...
                const done = () => {
                    ...
                    li.removeAttribute('data-draggable');  // ← ADD THIS
        ...
        const HistoryManager = {
            // Configuration constants
            ICONS: {
                true: '<span class="typeicon green">✓</span>',
                false: '<span class="typeicon red">✗</span>',
                price: '<span class="typeicon price">$</span>',
                added: '<span style="font-family: \'Segoe UI Symbol\', sans-serif;" class="typeicon green">➕</span>',
                removed: '<span style="font-family: \'Segoe UI Symbol\', sans-serif;" class="typeicon red">➖</span>'
            },
            ...
            renderPage(container, direction = 'none') {
                ...
                this.historyData
                    .slice(this.currentPage * this.MAX_ENTRIES_PER_PAGE, (this.currentPage + 1) * this.MAX_ENTRIES_PER_PAGE)
                    .forEach(entry => this.renderEntry(entry, newInner));
            ...
            formatPriceItem(item) {
                return `${this.ICONS.price} <strong>[${item.item}]</strong> <span class="old strike">${item.oldPrice}</span> → <span class="new">${item.newPrice}</span>`;
            },

            formatpoopItem(item) {
                return `<strong>[${item.item}]</strong> <span class="old strike">${item.oldDesc}</span> → <span class="new">${item.newDesc}</span>`;
            },
            ...
            formatDemandItem(item) {
                ...
                const starIcon = '<span class="typeicon" style="color: #ffd700;">★</span>';
                return `${starIcon} <strong>[${item.item}]</strong> <span class="old strike">${oldLabel} (${oldDemand}★)</span> → <span class="new">${newLabel} (${newDemand}★)</span>`;
            },
            ...
            formatDescriptionItem(item) {
                ...
                for (let i = 0; i < maxLines; i++) {
                    ...
                    if (oldLine === newLine) {
                        ...
                        output += `&nbsp;&nbsp;⨽&nbsp;<span class="amber">${oldLine}</span> → <span class="amber">${newLine}</span><br>`;
                    } else if (oldLine && !newLine) {
                        output += `&nbsp;&nbsp;⨽&nbsp;<span class="old">${oldLine} 🗙</span><br>`;
                    } else if (!oldLine && newLine) {
                        output += `&nbsp;&nbsp;⨽&nbsp;<span class="new">${newLine} +</span><br>`;
                    }
                }
            ...
            formatImageItem(item) {
                ...
                if (oldFile === newFile) {
                    ...
                    return `<strong>[${item.item}]</strong> moved: ${oldDomain} → ${newDomain}`;
                }

                return `<strong>[${item.item}]</strong> <span class="old">${oldFile}</span> → <span class="new">${newFile}</span>`;
            },

            formatSvgItem(item) {
                return `<strong>[${item.item}]</strong> updated SVG`;
            },
            ...
            processChanges(diffLines) {
                ...
                for (const line of diffLines) {
                    ...
                    const removedMatch = line.match(/^❌\s+Removed:\s+"([^"]+)"$/);
                    if (removedMatch) {
                    ...
                    if (this.BOOLEAN_FIELDS.includes(field) && (newRaw === 'true' || newRaw === 'false')) {
                        ...
                        changes.credits.push({ item, oldDesc: oldRaw || '', newDesc: newRaw || '' });
                    } else if (field === 'lore') {
                        changes.lore.push({ item, oldDesc: oldRaw || '', newDesc: newRaw || '' });
                    } else if (field === 'alias') {
                        changes.alias.push({ item, oldDesc: oldRaw || '', newDesc: newRaw || '' });
                    } else if (field === 'quantity') {
                        changes.quantity.push({ item, oldDesc: oldRaw || '', newDesc: newRaw || '' });
                    }
                }
            ...
            renderEntry(entry, container) {
                ...
                const formatters = {
                    ...
                    svg: this.formatSvgItem,  // Simple formatter for SVG
                    price: this.formatPriceItem,
                    from: this.formatDescriptionItem,
                    'price/code/rarity': this.formatDescriptionItem,
                    credits: this.formatpoopItem,
                    lore: this.formatpoopItem,
                    alias: this.formatpoopItem,
                    quantity: this.formatpoopItem
                };
                ...
                if (!anySectionAdded) {
                    ...
                    if (entry.diff && (entry.diff.includes('🆕 Added:') || entry.diff.includes('❌ Removed:'))) {
        ...
        const JSONImportManager = {
            ...
            `;
                ...
                <div style="padding: 20px; border-bottom: 1px solid var(--shade, #374151); 
                    ...
                    <p style="margin: 4px 0 0 0; color: #3b82f6; font-size: 13px;">
                        ℹ️ You'll need to publish after importing to save changes to the server
                    </p>
                </div>
                ...
                <div style="padding: 20px; overflow-y: auto; flex: 1;">
                    <div style="background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); 
                        ...
                        <div style="color: #d1d5db; font-size: 14px;">
                            ...
                            <div>• New Items: ${stats.newItems}</div>
                            <div>• Removed Items: ${stats.removedItems}</div>
                            <div>• Unchanged Items: ${stats.unchangedItems}</div>
                        </div>
                    </div>
                    ...
                    <div style="background: rgba(251, 191, 36, 0.1); border: 1px solid rgba(251, 191, 36, 0.3); 
                        ...
                        <div style="color: #9ca3af; margin-top: 4px; font-size: 14px;">
                            • ${state.allItems.length} total items
            ...
            applyImportedData(data) {
                ...
                if (totalChanges > 0) {
                    ErrorHandler.showToast(`✓ Imported with ${totalChanges} change${totalChanges !== 1 ? 's' : ''} ready to publish`, 'success');
                };
            }
        };
        // ============================
  ...
  <path d="M 79.291 80.773 C 79.291 79.668 80.186 78.773 81.291 78.773 L 87.291 78.773 C 88.396 78.773 89.291 79.668 89.291 80.773 L 89.291 82.773 L 93.291 82.773 C 94.061 82.773 94.542 83.606 94.157 84.273 C 93.978 84.582 93.648 84.773 93.291 84.773 L 92.222 84.773 L 91.355 96.915 C 91.281 97.961 90.409 98.773 89.36 98.773 L 79.221 98.773 C 78.172 98.773 77.301 97.961 77.226 96.915 L 76.361 84.773 L 75.291 84.773 C 74.521 84.773 74.04 83.939 74.425 83.273 C 74.604 82.963 74.934 82.773 75.291 82.773 L 79.291 82.773 L 79.291 80.773 Z M 81.291 82.773 L 87.291 82.773 L 87.291 80.773 L 81.291 80.773 L 81.291 82.773 Z M 78.365 84.773 L 79.222 96.773 L 89.361 96.773 L 90.218 84.773 L 78.365 84.773 Z M 82.291 86.773 C 82.843 86.773 83.291 87.22 83.291 87.773 L 83.291 93.773 C 83.291 94.542 82.458 95.024 81.791 94.639 C 81.482 94.46 81.291 94.13 81.291 93.773 L 81.291 87.773 C 81.291 87.22 81.739 86.773 82.291 86.773 Z M 86.291 86.773 C 86.843 86.773 87.291 87.22 87.291 87.773 L 87.291 93.773 C 87.291 94.542 86.458 95.024 85.791 94.639 C 85.482 94.46 85.291 94.13 85.291 93.773 L 85.291 87.773 C 85.291 87.22 85.739 86.773 86.291 86.773 Z" style="stroke-width: 1;" transform="matrix(1, 0, 0, 1, 7.105427357601002e-15, 1.4210854715202004e-14)"/>
        ...
        const PublishManager = {
            ...
            compareItems(original, current) {
                ...
                fields.forEach(field => {
                    // Use hasOwnProperty to check if field exists, or get undefined
                    const oldVal = this.normalizeValue(field, original.hasOwnProperty(field) ? original[field] : undefined);
                    const newVal = this.normalizeValue(field, current.hasOwnProperty(field) ? current[field] : undefined);

                    if (oldVal !== newVal) {
                        changes.push({
                            field,
                            oldValue: original.hasOwnProperty(field) ? original[field] : undefined,
                            newValue: current.hasOwnProperty(field) ? current[field] : undefined,
                            display: this.formatFieldChange(field, original.hasOwnProperty(field) ? original[field] : undefined, current.hasOwnProperty(field) ? current[field] : undefined)
                        });
                    }
                });

                // Price history (edited via the Price History editor) isn't a scalar field, so
                // compare it separately. Without this a history-only edit yields an empty diff and
                // the item silently drops out of the publish summary ("No changes to publish").
                const phStr = (arr) => JSON.stringify(
                    (Array.isArray(arr) ? arr : []).map(p => ({
                        price: Number(p.price) || 0,
                        timestamp: Number(p.timestamp) || 0
                ...
                if (phStr(original.priceHistory) !== phStr(current.priceHistory)) {
                    const oldCount = Array.isArray(original.priceHistory) ? original.priceHistory.length : 0;
                    const newCount = Array.isArray(current.priceHistory) ? current.priceHistory.length : 0;
                    changes.push({
                        field: 'priceHistory',
                        oldValue: oldCount,
                        newValue: newCount,
                        display: `Price history updated (${oldCount} → ${newCount} point${newCount !== 1 ? 's' : ''})`
                    });
                }
            ...
            normalizeValue(field, value) {
                ...
                // Normalize demand_updated_at for comparison (handle null/undefined)
                if (field === 'demand_updated_at') {
            ...
            formatFieldChange(field, oldValue, newValue) {
                const formatters = {
                    'img': (old, val) => {
                        ...
                        return `Image: ${getFilename(old)} → ${getFilename(val)}`;
                    },
                    'svg': (old, val) => `SVG code updated`,
                    'price': (old, val) => `RAP: ${old || '0'} → ${val || '0'}`,
                    'from': (old, val) => `Description changed`,
                    'price/code/rarity': (old, val) => `Price/Code/Rarity changed`,
                    'weekly': (old, val) => val ? '✓ Added to Weekly Shop' : '✗ Removed from Weekly Shop',
                    'weeklystar': (old, val) => val ? '✓ Added to Star Shop' : '✗ Removed from Star Shop',
                    ...
                    'demand': (old, val) => {
                        ...
                        return `Demand: ${oldLabel} (${old || 0}★) → ${newLabel} (${val || 0}★)`;
                    },
                    'demand_updated_at': (old, val) => {
                        const formatDate = (timestamp) => {
                            if (!timestamp) return 'Never';
                            const date = new Date(typeof timestamp === 'number' 
                                ? (timestamp < 10000000000 ? timestamp * 1000 : timestamp)
                                : timestamp);
                            return date.toLocaleDateString();
                        };
                        return `Demand updated: ${formatDate(old)} → ${formatDate(val)}`;
                    },
                    'credits': (old, val) => {
                        const oldText = old ? (old.length > 30 ? old.substring(0, 30) + '...' : old) : '(empty)';
                        const newText = val ? (val.length > 30 ? val.substring(0, 30) + '...' : val) : '(empty)';
                        return `Credits: "${oldText}" → "${newText}"`;
                    },
                    'lore': (old, val) => {
                        const oldText = old ? (old.length > 30 ? old.substring(0, 30) + '...' : old) : '(empty)';
                        const newText = val ? (val.length > 30 ? val.substring(0, 30) + '...' : val) : '(empty)';
                        return `Lore: "${oldText}" → "${newText}"`;
                    },
                    'alias': (old, val) => {
                        const oldText = old ? (old.length > 30 ? old.substring(0, 30) + '...' : old) : '(empty)';
                        const newText = val ? (val.length > 30 ? val.substring(0, 30) + '...' : val) : '(empty)';
                        return `Alias: "${oldText}" → "${newText}"`;
                    },
                    'quantity': (old, val) => {
                        const oldText = old ? (old.length > 30 ? old.substring(0, 30) + '...' : old) : '(empty)';
                        const newText = val ? (val.length > 30 ? val.substring(0, 30) + '...' : val) : '(empty)';
                        return `Quantity: "${oldText}" → "${newText}"`;
                    }
                };

                return formatters[field] ? formatters[field](oldValue, newValue) : `${field}: ${oldValue} → ${newValue}`;
            },
            ...
            renderChanges(changes) {
                ...
                <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); 
                    ...
                    changes.added.forEach(item => {
                        html += `<div style="color: #d1d5db; margin: 4px 0;">• ${item.name}</div>`;
                    });
                ...
                try {
                    ...
                    if (response.status === 409) {
                        ...
                        if (forceCheckbox && forceCheckbox.checked) {
                            // Force overwrite - reload items first to get latest updated_at, then retry
                            ErrorHandler.showToast('Reloading latest data...', 'info');
                            await API.loadData();
                            ChangeTracker.initializeOriginals();
                            
                            const forceResponse = await API.saveData();
                        ...
                        try {
                            ...
                            // Show detailed error if there are specific item errors
                            if (errorJson.errors && errorJson.errors.length > 0) {
                                const errorList = errorJson.errors.slice(0, 5).map(e => 
                                    `• ${e.item}: ${e.error}`
                                ).join('\n');
                                errorMessage = `${errorMessage}\n\n${errorList}${errorJson.errors.length > 5 ? `\n... and ${errorJson.errors.length - 5} more` : ''}`;
                            }
                        } catch (e) {
        ...
        const App = {
            async init() {
                ...
                // UpdateChecker.start(); // Disabled - no longer needed with D1 database
            ...
            handleUpdate(updatedBy, newVersion) {
                ...
                if (this.hasUnsavedChanges()) {
                    ...
                    ErrorHandler.showToast(
                        `📦 New catalog update by ${updatedBy}. Reloading in 5 seconds...`,
                        'info'
                    );
    ...
    font-size: 25px;">
            ...
            <p style="color: #9ca3af; margin: 12px 0 0 0; font-size: 12px;">
                <span id="countdown">Auto-reload paused</span> • Version: ${newVersion}
            </p>
        `;
        ...
        `;
                badge.innerHTML = `📦 Update available from ${updatedBy}`;
        ...
        // ============================
        // Replaces a 650-line hand-rolled sticky+drag state machine with:
        //   - native CSS `position: sticky` for sidebar mode (see #quick-editor-content.mode-sidebar in admin.css)
        //   - interact.js for the free-floating modal drag
        //   - a CSS media-query rule for mobile full-screen
        // Public API (init, show, hide, refreshOverflow, state.isInitialized,
        // state.isMobile) is preserved so existing call sites keep working.
        const DraggablePanelManager = {
            ...
            setupEventListeners() {
                ...
                const debouncedResize = (typeof Utils !== 'undefined' && Utils.debounce)
                    ...
                    : (() => {
                        let t;
                        return () => { clearTimeout(t); t = setTimeout(() => this.handleResize(), 150); };
                    })();
                ...
                // Re-apply mode whenever the host modal toggles visibility.
                this.state.modalObserver = new MutationObserver(() => {
                    if (this.isVisible()) requestAnimationFrame(() => this.handleResize());
                });
                this.state.modalObserver.observe(this.elements.modal, {
                    attributes: true,
            ...
            handleResize() {
                ...
                if (this.state.isMobile) return this.applyMode('mobile');
                if (window.innerWidth > this.config.WIDTH_THRESHOLD) return this.applyMode('sidebar');
                return this.applyMode('modal');
            },

            applyMode(mode) {
                if (this.state.currentMode === mode) return;
                const { panel, modal, mainEl, spacer, closeBtn } = this.elements;
                if (!panel || !modal || !mainEl || !spacer) return;

                // Clear previous mode's inline styles + class.
                panel.classList.remove('mode-sidebar', 'mode-modal', 'mode-mobile');
                panel.style.cssText = '';

                // Tear down drag if leaving modal mode.
                if (this.state.currentMode === 'modal' && this.state.interactable) {
                    this.state.interactable.unset();
                    this.state.interactable = null;
                }
                ...
                if (mode === 'sidebar') {
                    ...
                    panel.classList.add('mode-sidebar');
                } else if (mode === 'modal') {
                    if (!modal.contains(panel)) modal.appendChild(panel);
                    spacer.style.display = 'none';
                    ...
                    spacer.style.display = 'none';
                    if (closeBtn) closeBtn.style.display = '';
                    panel.classList.add('mode-mobile');
                }
            ...
            centerPanel() {
                ...
                if (!panel) return;
                // Defer to next frame so offsetWidth/Height reflect mode-modal styles.
                requestAnimationFrame(() => {
                    const w = panel.offsetWidth;
                    panel.style.left = Math.max(this.config.SNAP_MARGIN, Math.round((window.innerWidth - w) / 2)) + 'px';
                    panel.style.top = Math.max(this.config.SNAP_MARGIN, Math.round(window.innerHeight * 0.1)) + 'px';
                });
            },

            enableDrag() {
                if (typeof interact === 'undefined') {
                    console.warn('interact.js not loaded - draggable mode disabled');
                    return;
                }
                const { panel } = this.elements;
                const margin = this.config.SNAP_MARGIN;

                this.state.interactable = interact(panel).draggable({
                    inertia: true,
                    modifiers: [
                    ...
                    allowFrom: '#quick-editor-title, #quick-editor-img, #tutorial',
                    ignoreFrom: 'input, textarea, button, select, [contenteditable]',
                    listeners: {
                        start: () => panel.classList.add('dragging'),
                        move: (event) => {
                            const left = parseFloat(panel.style.left) || 0;
                            const top = parseFloat(panel.style.top) || 0;
                            panel.style.left = (left + event.dx) + 'px';
                            panel.style.top = (top + event.dy) + 'px';
                        },
                        end: () => {
                            panel.classList.remove('dragging');
                            // Edge snap to nearest horizontal edge.
            ...
            // Native `position: sticky` handles what this used to compute.
            refreshOverflow() {},
        ...
        const ChestManager = {
            ...
            bindEvents() {
                const addBtn = document.getElementById('add-chest-btn');
                const overlay = document.getElementById('chest-editor-overlay');
                const cancelBtn = document.getElementById('chest-ed-cancel');
                const saveBtn = document.getElementById('chest-ed-save');
                const metaTypeSelect = document.getElementById('chest-ed-metatype');
                const imgInput = document.getElementById('chest-ed-img');

                addBtn.addEventListener('click', () => this.openEditor());
                cancelBtn.addEventListener('click', () => this.closeEditor());
                saveBtn.addEventListener('click', () => this.saveChest());
                overlay.addEventListener('click', (e) => {
                    if (e.target === overlay) this.closeEditor();
                });
                metaTypeSelect.addEventListener('change', () => {
                    document.getElementById('chest-ed-custom-fields').style.display =
                        metaTypeSelect.value === 'custom' ? 'flex' : 'none';
                });
                imgInput.addEventListener('input', () => {
                    const preview = document.getElementById('chest-ed-img-preview');
            ...
            async loadChests() {
                try {
                    ...
                    document.getElementById('chest-management-list').innerHTML =
                        '<p class="loading" style="color:#ff6b6b">Failed to load chests</p>';
                }
            },

            render() {
                const container = document.getElementById('chest-management-list');
                const countLabel = document.getElementById('chest-count-label');
                countLabel.textContent = `${this.chests.length} chest${this.chests.length !== 1 ? 's' : ''}`;

                if (this.chests.length === 0) {
                    container.innerHTML = '<p class="loading">No chests defined yet</p>';
                    return;
                }

                const metaTypeLabels = { timer: 'Timer', coins: 'Coins', robux: 'Robux', text: 'Text', custom: 'Custom' };

                container.innerHTML = this.chests.map(c => `
                    <div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:#63636320;border:1px solid #6363633d;border-radius:8px;transition:background .2s"
                         onmouseenter="this.style.background='#63636340'" onmouseleave="this.style.background='#63636320'">
                        <img src="${this.escHtml(c.image_url)}" alt="${this.escHtml(c.name)}"
                             style="width:38px;height:38px;object-fit:contain;filter:drop-shadow(0 2px 4px rgba(0,0,0,.5));flex-shrink:0${c.rainbow_effect ? ';animation:2s rainbow-chest infinite linear' : ''}">
                        <div style="flex:1;min-width:0">
                            <div style="font-weight:600;font-size:14px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${this.escHtml(c.name)}</div>
                            <div style="font-size:11px;color:#999;display:flex;gap:8px;flex-wrap:wrap;margin-top:2px">
                                <span style="color:#667eea">${this.escHtml(c.id)}</span>
                                <span>${metaTypeLabels[c.meta_type] || c.meta_type}: ${this.escHtml(c.meta_value || '—')}</span>
                                <span style="color:#666">keywords: ${this.escHtml(c.from_keywords)}</span>
                            </div>
                        </div>
                        <div style="display:flex;gap:6px;flex-shrink:0">
                            <button class="small-btn" style="padding:5px 12px;font-size:12px"
                                    onclick="ChestManager.openEditor('${this.escHtml(c.id)}')">Edit</button>
                            <button class="small-btn" style="padding:5px 12px;font-size:12px;background:#c0392b"
                                    onclick="ChestManager.confirmDelete('${this.escHtml(c.id)}','${this.escHtml(c.name)}')">Delete</button>
                        </div>
                    </div>
                `).join('');
            },
            ...
            openEditor(id) {
                ...
                const overlay = document.getElementById('chest-editor-overlay');
                const title = document.getElementById('chest-editor-title');
                const idInput = document.getElementById('chest-ed-id');

                if (id) {
                    const chest = this.chests.find(c => c.id === id);
                    if (!chest) return;
                    title.textContent = 'Edit Chest';
                    idInput.value = chest.id;
                    idInput.disabled = true;
                    idInput.style.opacity = '0.5';
                    document.getElementById('chest-ed-name').value = chest.name;
                    document.getElementById('chest-ed-img').value = chest.image_url;
                    document.getElementById('chest-ed-keywords').value = chest.from_keywords;
                    document.getElementById('chest-ed-metatype').value = chest.meta_type;
                    document.getElementById('chest-ed-metaval').value = chest.meta_value || '';
                    document.getElementById('chest-ed-metaicon').value = chest.meta_icon || '';
                    document.getElementById('chest-ed-metastyle').value = chest.meta_style || '';
                    document.getElementById('chest-ed-sort').value = chest.sort_order;
                    document.getElementById('chest-ed-rainbow').checked = !!chest.rainbow_effect;
                    document.getElementById('chest-ed-custom-fields').style.display =
                        chest.meta_type === 'custom' ? 'flex' : 'none';
                    const preview = document.getElementById('chest-ed-img-preview');
                    preview.src = chest.image_url;
                    preview.style.display = 'block';
                } else {
                    title.textContent = 'Add Chest';
                    idInput.disabled = false;
                    idInput.style.opacity = '1';
                    idInput.value = '';
                    document.getElementById('chest-ed-name').value = '';
                    document.getElementById('chest-ed-img').value = '';
                    document.getElementById('chest-ed-keywords').value = '';
                    document.getElementById('chest-ed-metatype').value = 'text';
                    document.getElementById('chest-ed-metaval').value = '';
                    document.getElementById('chest-ed-metaicon').value = '';
                    document.getElementById('chest-ed-metastyle').value = '';
                    document.getElementById('chest-ed-sort').value = this.chests.length;
                    document.getElementById('chest-ed-rainbow').checked = false;
                    document.getElementById('chest-ed-custom-fields').style.display = 'none';
                    document.getElementById('chest-ed-img-preview').style.display = 'none';
                }
                overlay.style.display = 'flex';
            },

            closeEditor() {
                document.getElementById('chest-editor-overlay').style.display = 'none';
                this.editingId = null;
            },

            async saveChest() {
                const payload = {
                    id: document.getElementById('chest-ed-id').value.trim(),
                    name: document.getElementById('chest-ed-name').value.trim(),
                    image_url: document.getElementById('chest-ed-img').value.trim(),
                    from_keywords: document.getElementById('chest-ed-keywords').value.trim(),
                    meta_type: document.getElementById('chest-ed-metatype').value,
                    meta_value: document.getElementById('chest-ed-metaval').value.trim() || null,
                    meta_icon: document.getElementById('chest-ed-metaicon').value.trim() || null,
                    meta_style: document.getElementById('chest-ed-metastyle').value.trim() || null,
                    sort_order: parseInt(document.getElementById('chest-ed-sort').value) || 0,
                    rainbow_effect: document.getElementById('chest-ed-rainbow').checked
                };

                if (!payload.id || !payload.name || !payload.image_url || !payload.from_keywords) {
                    ErrorHandler.showToast('Please fill in all required fields (ID, Name, Image URL, Keywords)', 'error');
                    return;
                }
                ...
                try {
                    ...
                    const url = isEdit ? `/api/chests/${this.editingId}` : '/api/chests';
                    ...
                    const data = await res.json();
                    if (!res.ok) {
                        ErrorHandler.showToast(data.error || 'Failed to save chest', 'error');
                        return;
                    }

                    ErrorHandler.showToast(isEdit ? 'Chest updated' : 'Chest created', 'success');
                    this.closeEditor();
                    await this.loadChests();
                } catch (e) {
                    console.error('Save chest error:', e);
                    ErrorHandler.showToast('Failed to save chest', 'error');
                } finally {
                    saveBtn.disabled = false;
                    saveBtn.textContent = 'Save Chest';
                }
            },
            ...
            async confirmDelete(id, name) {
                ...
                const token = localStorage.getItem('auth_token');
                try {
                    const res = await fetch(`/api/chests/${id}`, {
                        method: 'DELETE',
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    ...
                    if (!res.ok) {
                        const data = await res.json();
                        ErrorHandler.showToast(data.error || 'Failed to delete chest', 'error');
                        return;
                    }

                    ErrorHandler.showToast('Chest deleted', 'success');
                    await this.loadChests();
                } catch (e) {
                    console.error('Delete chest error:', e);
                    ErrorHandler.showToast('Failed to delete chest', 'error');
                }
            },
        ...
        const GalleryMod = {
            ...
            async loadPendingItems() {
                ...
                try {
                    const response = await fetch('/api/gallery/pending', {
                        headers: {
                    ...
                    if (items.length === 0) {
                        ...
                        document.getElementById('gallery-moderation-section').style.display = 'none';
                        return;
                    }
        ...
        const UserManagement = {
            ...
            async searchUser() {
                ...
                try {
                    ...
                    const response = await fetch(`/api/auth/user/search?q=${encodeURIComponent(query)}`, {
                        headers: {
                            'Authorization': `Bearer ${Auth.session}`
                        }
                    });
            ...
            if (reviewSection) {
                const observer = new MutationObserver((mutations) => {
                    if (reviewSection.style.display !== 'none') {
                        // Reload items when section becomes visible (in case data was loaded after initial check)
                        DemandReview.init();
                    }
                });
    ...
    `;
        ...
        const DemandReview = {
            ...
            minimizedItems: new Set(), // Track minimized items by "category:name"

            async init() {
                // Wait for data to be loaded if not ready
                if (!state.jsData || Object.keys(state.jsData).length === 0) {
                    ...
                    if (!state.jsData || Object.keys(state.jsData).length === 0) {
                        const grid = document.getElementById('demand-items-grid');
                        if (grid) {
                            grid.innerHTML = '<p class="error">No items loaded. Please refresh the page.</p>';
                        }
                        return;
                    }
                }
            ...
            loadReviewItems() {
                this.reviewItems = [];
                const now = Date.now();
                const thirtyDaysAgo = now - (1 * 24 * 60 * 60 * 1000); // 30 days in milliseconds

                // Iterate through all categories in state.jsData
                for (const [category, items] of Object.entries(state.jsData)) {
                    if (!Array.isArray(items)) continue;

                    for (const item of items) {
                        const demand = item.demand ?? 0;
                        const demandUpdatedAt = item.demand_updated_at;

                        // Check if item needs review
                        let hasNoRating = false;
                        let isOld = false;

                        // No rating: demand is 0 or null/undefined
                        if (demand === 0 || demand === null || demand === undefined) {
                            hasNoRating = true;
                        }

                        // Old rating: demand_updated_at is older than 30 days or null/undefined
                        if (demand > 0) {
                            if (!demandUpdatedAt) {
                                isOld = true;
                            } else {
                                // Convert Unix timestamp (seconds) to milliseconds
                                const updatedAtMs = typeof demandUpdatedAt === 'number' 
                                    ? (demandUpdatedAt < 10000000000 ? demandUpdatedAt * 1000 : demandUpdatedAt)
                                    : new Date(demandUpdatedAt).getTime();
                                
                                if (updatedAtMs > thirtyDaysAgo) {
                                    isOld = true;
                                }
                            }
                        }
                        ...
                        if (hasNoRating || isOld) {
                            this.reviewItems.push({
                                ...
                                isOld: isOld,
                                updatedAt: demandUpdatedAt ? (
                                    typeof demandUpdatedAt === 'number'
                                        ? (demandUpdatedAt < 10000000000 ? demandUpdatedAt * 1000 : demandUpdatedAt)
                                        : new Date(demandUpdatedAt).getTime()
                                ) : null
                            });
                ...
                // Sort by priority: no rating first, then by how old the rating is
                this.reviewItems.sort((a, b) => {
                    if (a.hasNoRating && !b.hasNoRating) return -1;
                    if (!a.hasNoRating && b.hasNoRating) return 1;
                    if (a.isOld && !b.isOld) return -1;
                    if (!a.isOld && b.isOld) return 1;
                    // Both have same priority, sort by update date (oldest first)
                    const aTime = a.updatedAt || 0;
                    const bTime = b.updatedAt || 0;
                    return aTime - bTime;
                });

                console.log(`Loaded ${this.reviewItems.length} items needing review`);
            },
            ...
            renderItems() {
                ...
                if (this.reviewItems.length === 0) {
                    grid.innerHTML = '<p class="loading">No items need review. All items have up-to-date demand ratings!</p>';
                } else if (this.currentItems.length === 0) {
                    grid.innerHTML = '<p class="loading">No items match the current filters. Try adjusting your search or filter options.</p>';
                } else {
                ...
                // Setup event handlers for star ratings and buttons
                pageItems.forEach(item => {
                    this.setupItemHandlers(item);
                });
            },

            setupItemHandlers(item) {
                const card = document.getElementById(`review-card-${item.category}-${item.name}`);
                if (!card) return;

                const itemKey = `${item.category}:${item.name}`;

                // Setup star rating clicks and hover effects
                const stars = card.querySelectorAll('.demand-star');
                stars.forEach((star) => {
                    ...
                    star.addEventListener('click', (e) => {
                        e.stopPropagation();
                        this.updateDemandRating(item, rating);
                    });
                    ...
                    star.addEventListener('mouseenter', () => {
                        stars.forEach((s, idx) => {
                            ...
                            if (sRating <= rating) {
                                ...
                                s.style.color = this.getStarColor(rating);
                            }
                        });
                    });

                    star.addEventListener('mouseleave', () => {
                        const currentRating = item.demand || 0;
                        stars.forEach((s, idx) => {
                            const sRating = idx + 1;
                            s.style.transform = 'scale(1)';
                            s.style.color = sRating <= currentRating 
                                ? this.getStarColor(currentRating) 
                                : '#666';
                        });
                    });
                });

                // Setup "Keep Demand" button
                const keepBtn = card.querySelector('.keep-demand-btn');
                if (keepBtn) {
                    keepBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        this.keepDemandRating(item);
                    });
                }

                // Setup expand/collapse toggle
                const expandBtns = card.querySelectorAll('.expand-toggle-btn');
                expandBtns.forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        this.toggleMinimize(item);
                    });
                });
            },
            ...
            getStarColor(rating) {
                ...
                return demandColors[rating] || '#FFD700';
            },
            ...
            renderItemCard(item) {
                ...
                const itemKey = `${item.category}:${item.name}`;
                const isMinimized = this.minimizedItems.has(itemKey);
                const currentRating = item.demand || 0;
                ...
                const starsHtml = Array.from({ length: 5 }, (_, i) => {
                    ...
                    const starColor = isFilled ? (demandColors[currentRating] || '#FFD700') : '#666';
                    return `
                        <span class="demand-star" data-rating="${starValue}" data-item-key="${itemKey}" style="
                            font-size: 24px;
                            color: ${starColor};
                            cursor: pointer;
                            user-select: none;
                            ...
                            display: inline-block;
                            line-height: 1;
                        " title="Rating: ${starValue} - ${demandLabels[starValue]}">${isFilled ? '★' : '☆'}</span>
                    `;
                }).join('');

                const expandedContent = `
                    <div class="demand-rating-section" style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #6363637d;">
                        <div style="margin-bottom: 8px;">
                            <div style="font-size: 12px; color: var(--text-secondary); margin-bottom: 6px;">Current Rating:</div>
                            <div style="display: flex; align-items: center; gap: 8px;">
                                ${starsHtml}
                                <span style="font-size: 12px; color: ${demandColors[currentRating]}; margin-left: 8px;">${demandLabels[currentRating]}</span>
                            </div>
                        </div>
                        <div style="display: flex; gap: 8px; margin-top: 12px;">
                            <button class="keep-demand-btn" style="
                                flex: 1;
                                padding: 8px 16px;
                                border: 1px solid #6363637d;
                                background: #63636347;
                                ...
                                font-weight: 500;
                                transition: all 0.2s;
                            " onmouseover="this.style.background='#63636366'; this.style.borderColor='#3b82f6'" onmouseout="this.style.background='#63636347'; this.style.borderColor='#6363637d'">Keep Demand</button>
                        </div>
                    </div>
                `;

                const minimizedStarsHtml = Array.from({ length: 5 }, (_, i) => {
                    const starValue = i + 1;
                    const isFilled = starValue <= currentRating;
                    const starColor = isFilled ? (demandColors[currentRating] || '#FFD700') : '#666';
                    return `<span style="font-size: 14px; color: ${starColor};">${isFilled ? '★' : '☆'}</span>`;
                }).join('');

                const minimizedContent = `
                    <div style="display: flex; align-items: center; gap: 8px; margin-top: 8px; padding-top: 8px; border-top: 1px solid #6363637d;">
                        <span style="font-size: 11px; color: var(--text-secondary);">✓ Reviewed</span>
                        <div style="display: flex; align-items: center; gap: 2px; margin-left: 8px;">
                            ${minimizedStarsHtml}
                        </div>
                        <span style="font-size: 11px; color: ${demandColors[currentRating]}; margin-left: 4px;">${demandLabels[currentRating]}</span>
                        <button class="expand-toggle-btn" style="
                            margin-left: auto;
                            padding: 4px 8px;
                            border: 1px solid #6363637d;
                ...
                return `
                    ...
                    ">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                            ...
                            ${isMinimized ? '' : `<button class="expand-toggle-btn" style="
                                padding: 4px 8px;
                                border: 1px solid #6363637d;
                                background: transparent;
                                color: #999;
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 8px;">
                            <div>
                                <div style="font-size: 11px; color: var(--text-secondary);">Current: <strong style="color: ${demandColors[currentRating]};">${demandLabels[currentRating]}</strong></div>
            ...
            updateDemandRating(item, newRating) {
                const actualItem = this.findItemInState(item.name, item.category);
                if (!actualItem) {
                    console.error('Item not found in state:', item.name);
                    ErrorHandler.showToast('Item not found', 'error');
                    return;
                }

                // Save previous state for undo/redo
                const previousState = JSON.parse(JSON.stringify(actualItem));
                const previousDemand = actualItem.demand || 0;

                // Update the item in state
                actualItem.demand = newRating;
                
                // Mark as modified for saving (will be saved when Publish is clicked)
                ChangeTracker.checkModifications(actualItem);

                // Update the review item
                item.demand = newRating;
                item.hasNoRating = false;
                item.isOld = false;

                // Record change for undo/redo
                if (!UndoRedoManager.isApplyingChange && previousDemand !== newRating) {
                    UndoRedoManager.recordChange({
                        type: 'edit',
                        itemName: actualItem.name,
                        category: actualItem.category,
                        previousState: previousState,
                        newState: JSON.parse(JSON.stringify(actualItem))
                    });
                }

                // Update Quick Editor if it's open for this item
                // state.currentEditingItem should be the same reference as actualItem, but update it to be sure
                if (state.currentEditingItem && state.currentEditingItem.name === actualItem.name) {
                    // Update the currentEditingItem reference to match actualItem
                    state.currentEditingItem.demand = actualItem.demand;
                    state.currentEditingItem.demand_updated_at = actualItem.demand_updated_at;
                    // Update Quick Editor UI
                    QuickEditor.setupDemandRating(state.currentEditingItem);
                }
                ...
                ErrorHandler.showToast(`Demand rating updated to ${newRating} (will save on Publish)`, 'success');
            },

            keepDemandRating(item) {
                const actualItem = this.findItemInState(item.name, item.category);
                if (!actualItem) {
                    console.error('Item not found in state:', item.name);
                    ErrorHandler.showToast('Item not found', 'error');
                ...
                // Save previous state for undo/redo
                const previousState = JSON.parse(JSON.stringify(actualItem));
                const previousDemandUpdatedAt = actualItem.demand_updated_at;

                // Update demand_updated_at to current time (Unix timestamp in seconds)
                const now = Math.floor(Date.now() / 1000);
                actualItem.demand_updated_at = now;

                // Update the review item's updatedAt for display
                item.updatedAt = Date.now();

                // Force mark as modified - checkModifications should detect it, but ensure it's marked
                ChangeTracker.checkModifications(actualItem);
                
                // Double-check: if checkModifications didn't mark it, force it
                // This handles edge cases where the original might not have had demand_updated_at
                if (!ChangeTracker.isModifiedItem(actualItem.name)) {
                    console.log('Force marking item as modified (demand_updated_at changed):', actualItem.name, {
                        original: previousDemandUpdatedAt,
                        new: now
                    });
                    ChangeTracker.modifiedItems.add(actualItem.name);
                } else {
                    console.log('Item marked as modified:', actualItem.name);
                }

                // Record change for undo/redo
                if (!UndoRedoManager.isApplyingChange && previousDemandUpdatedAt !== now) {
                    UndoRedoManager.recordChange({
                        type: 'edit',
                        itemName: actualItem.name,
                        category: actualItem.category,
                ...
                if (state.currentEditingItem && state.currentEditingItem.name === actualItem.name) {
                    // Update the currentEditingItem reference to match actualItem
                    state.currentEditingItem.demand_updated_at = actualItem.demand_updated_at;
                    // Update Quick Editor UI if needed
                    QuickEditor.setupDemandRating(state.currentEditingItem);
                }

                // Update review item flags
                item.hasNoRating = false;
                item.isOld = false;
                ...
                ErrorHandler.showToast('Demand rating kept (will save on Publish)', 'success');
            },

            minimizeItem(item) {
                const itemKey = `${item.category}:${item.name}`;
                this.minimizedItems.add(itemKey);
                this.renderItems();
            },

            toggleMinimize(item) {
                const itemKey = `${item.category}:${item.name}`;
                if (this.minimizedItems.has(itemKey)) {
                    this.minimizedItems.delete(itemKey);
                } else {
                    this.minimizedItems.add(itemKey);
                }
                this.renderItems();
