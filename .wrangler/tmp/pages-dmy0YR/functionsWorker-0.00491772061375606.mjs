...
async function createNotification(env, userId, type, title, message, link = null) {
  try {
    ...
    ).bind(userId, type, title, message, link, Math.floor(Date.now() / 1e3)).run();
  } catch (e) {
  }
}
...
var init_path = __esm({
  "api/forum/comments/[[path]].js"() {
    init_functionsRoutes_0_41790263595957455();
...
async function handlePut2({ request, env, params }) {
  ...
  try {
    const data = await request.json();
    if (isAdmin2(user) && (data.is_pinned !== void 0 || data.is_locked !== void 0)) {
      if (data.is_pinned !== void 0) {
        await env.DBA.prepare("UPDATE forum_posts SET is_pinned = ? WHERE id = ?").bind(data.is_pinned ? 1 : 0, postId).run();
      }
      if (data.is_locked !== void 0) {
        await env.DBA.prepare("UPDATE forum_posts SET is_locked = ? WHERE id = ?").bind(data.is_locked ? 1 : 0, postId).run();
      }
    ...
    await env.DBA.prepare(
      `UPDATE forum_posts
       ...
       WHERE id = ?`
    ).bind(title, content, itemsJson, Math.floor(Date.now() / 1e3), postId).run();
    return new Response(JSON.stringify({
      success: true,
      message: "Post updated successfully!"
    }), {
...
var init_path2 = __esm({
  "api/forum/posts/[[path]].js"() {
    ...
    __name(getLikeCount2, "getLikeCount");
...
async function createNotification2(env, userId, type, title, message, link = null) {
  await env.DBA.prepare(
...
var init_helpers = __esm({
  "api/trades/_utils/helpers.js"() {
    ...
    __name(createNotification2, "createNotification");
...
async function handlePost3(request, env, path, user) {
  ...
  if (!path || path === "") {
    ...
    const validateItems = /* @__PURE__ */ __name((items, fieldName) => {
...
var init_path7 = __esm({
  "api/trades/reviews/[[path]].js"() {
    init_functionsRoutes_0_41790263595957455();
    init_helpers();
    __name(handleGet6, "handleGet");
    __name(handlePost7, "handlePost");
    __name(handlePut4, "handlePut");
    __name(handleDelete5, "handleDelete");
...
async function handleGet7({ request, env, params }) {
  ...
  const getUser = /* @__PURE__ */ __name(async () => {
...
var init_path8 = __esm({
  "api/gallery/[[path]].js"() {
    ...
    __name(getUserFromToken3, "getUserFromToken");
    __name(isAdmin3, "isAdmin");
    __name(shouldAutoApprove, "shouldAutoApprove");
    __name(handleGet7, "handleGet");
    __name(handlePost8, "handlePost");
    __name(handleDelete6, "handleDelete");
...
async function updateChest(id, request, env, corsHeaders) {
  ...
  const rainbow_effect = data.rainbow_effect !== void 0 ? data.rainbow_effect ? 1 : 0 : existing.rainbow_effect;
  await env.DBA.prepare(`
...
var init_path10 = __esm({
  "api/chests/[[path]].js"() {
    ...
    __name(listChests, "listChests");
    __name(createChest, "createChest");
    __name(updateChest, "updateChest");
    __name(deleteChest, "deleteChest");
  }
});
...
async function getHomepageItems(env, corsHeaders) {
  ...
  const transformItem = /* @__PURE__ */ __name((item) => ({
    ...item,
...
async function updateItem(id, request, env, corsHeaders) {
  ...
  const result = await env.DBA.prepare(`
    ...
    typicalgroup !== void 0 ? typicalgroup ? 1 : 0 : null,
...
async function getUserFromToken4(token, env) {
  if (!token) return null;
  const session = await env.DBA.prepare(
    "SELECT user_id FROM sessions WHERE token = ? AND expires_at > ?"
  ).bind(token, Date.now()).first();
...
async function onRequest16(context) {
  ...
  try {
    const authHeader = request.headers.get("Authorization");
    const token = authHeader?.replace("Bearer ", "");
    const user = await getUserFromToken4(token, env);
    if (!user) {
      const res = jsonResponse({ error: "Unauthorized" }, 401);
      const headers2 = new Headers(res.headers);
      Object.entries(corsHeaders).forEach(([k, v]) => headers2.set(k, v));
...
var init_path14 = __esm({
  "api/notifications/[[path]].js"() {
    init_functionsRoutes_0_41790263595957455();
    __name(getUserFromToken4, "getUserFromToken");
...
async function handleGetProfile(request, env) {
  ...
  try {
    const postsResult = await env.DBA.prepare(`
            ...
            LIMIT 12
        `).bind(actualUserId).all();
    const getMediaType2 = /* @__PURE__ */ __name((url2) => {
...
var init_roblox_proxy = __esm({
  "api/roblox-proxy.js"() {
    ...
    delay = /* @__PURE__ */ __name((ms) => new Promise((resolve) => setTimeout(resolve, ms)), "delay");
...
var init_donations = __esm({
  "api/donations.js"() {
    init_functionsRoutes_0_41790263595957455();
    __name(replaceUserTotals2, "replaceUserTotals");
...
async function onRequest20(context) {
  ...
  try {
    ...
    const getMediaType2 = /* @__PURE__ */ __name((url) => {
      if (!url) return "image";
...
// ../.wrangler/tmp/pages-dmy0YR/functionsRoutes-0.41790263595957455.mjs
var routes;
var init_functionsRoutes_0_41790263595957455 = __esm({
  "../.wrangler/tmp/pages-dmy0YR/functionsRoutes-0.41790263595957455.mjs"() {
    init_path();
    init_path2();
    init_path3();
    ...
    routes = [
      ...
      {
        routePath: "/api/chests/:path*",
        mountPath: "/api/chests",
        method: "",
        middlewares: [],
        modules: [onRequest12]
      },
      ...
      {
        routePath: "/api/notifications/:path*",
...
// ../.wrangler/tmp/bundle-6GQkxE/middleware-loader.entry.ts
init_functionsRoutes_0_41790263595957455();

// ../.wrangler/tmp/bundle-6GQkxE/middleware-insertion-facade.js
init_functionsRoutes_0_41790263595957455();
...
//# sourceMappingURL=functionsWorker-0.00491772061375606.mjs.map
