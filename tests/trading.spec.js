...
test.describe('Trading Hub page', () => {
    ...
    test('renders the page and all primary tabs', async ({ page }) => {
        ...
        await expect(page.locator('.tab-btn[data-tab="messages"]')).toBeVisible();
    ...
    test('has the new messages panel and review modal in the DOM', async ({ page }) => {
        await expect(page.locator('#panelMessages')).toHaveCount(1);
    ...
    test('gates My Trades behind login when logged out', async ({ page }) => {
        ...
        // Logged-out users are bounced back to Browse.
        await expect(page.locator('#panelBrowse')).toHaveClass(/active/);
    ...
    test('gates Messages behind login when logged out', async ({ page }) => {
        ...
        await expect(page.locator('#panelBrowse')).toHaveClass(/active/);
        await expect(page.locator('#panelMessages')).not.toHaveClass(/active/);
    });
